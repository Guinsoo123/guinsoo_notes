...
本目录（`doc/`）提供与官方 Sphinx 教程互补的**中文学习笔记**：用「一条主线 + PlantUML 图」把算法、软件架构、感知管线、运控管线和仿真/可视化闭环串起来。细节推导与 API 仍以官方文档为准。

**运控双读**：先掌握 API 与数据流见 [README_04](CH04_motion_control_pipeline.md)；要从**数学与优化直觉**分步推导到同一套 API，见 [README_06](CH06_motion_control_computational_walkthrough.md)。

## 推荐阅读顺序

| 顺序 | 文档 | 内容概要 |
|------|------|----------|
| 1 | [CH01_algorithm_design.md](CH01_algorithm_design.md) | 算法脉络：轨迹表示、距离场、IK、图规划、轨迹优化、MPC、动力学约束 |
| 2 | [CH02_software_design.md](CH02_software_design.md) | 软件分层：公开 API、`curobo._src`、Warp/CUDA、配置与类型 |
| 3 | [CH03_perception_pipeline.md](CH03_perception_pipeline.md) | 感知：`FilterDepth`、`Mapper`（TSDF/ESDF）、分割与位姿估计 |
| 4 | [CH04_motion_control_pipeline.md](CH04_motion_control_pipeline.md) | 运控 **API / 管线图**：模块表、示例路径、PlantUML |
| 5 | [CH06_motion_control_computational_walkthrough.md](CH06_motion_control_computational_walkthrough.md) | 运控 **计算方法导读（初学者）**：从 \(q\)、Jacobian、代价到 IK/TrajOpt/MPC 与 API 对应 |
| 6 | [CH05_simulation_visualization_closure.md](CH05_simulation_visualization_closure.md) | 外部仿真闭环、`ViserVisualizer`/`UsdWriter`、示例索引 |
| 7 | `DOC_RENDERING_GUIDE.md`（仓库根目录） | 仓库级文档维护指南：跨平台渲染工具、MkDocs 网页化、文档更新工作流 |
| 8 | `NOTES_REPO_STRUCTURE.md`（仓库根目录） | 仓库级结构规范：多项目目录模板、命名规则、链接策略、模板用法 |

## 与官方文档的关系

- **分步教程（英文）**：[Getting started](https://github.com/NVlabs/curobo/tree/main/docs/getting-started) — 安装、构型、FK、IK、MPC、运动规划、体素建图等。
- **概念深入**：[Concepts](https://github.com/NVlabs/curobo/tree/main/docs/concepts) — Rollout、优化器、几何图规划等。
- **论文与技术报告**：[Technical reports](https://github.com/NVlabs/curobo/blob/main/docs/technical_reports.rst) — cuRoboV2（[arXiv:2603.05493](https://arxiv.org/abs/2603.05493)）与 cuRobo v1（[arXiv:2310.17274](https://arxiv.org/abs/2310.17274)）。
...
2. **命令行**：下载 [PlantUML](https://plantuml.com/download)，执行  
   `java -jar plantuml.jar -tsvg doc/diagrams/*.puml`（若你单独保存了 `.puml` 文件）。
3. **Docker**：`docker run --rm -v "$PWD":/work plantuml/plantuml -tsvg /work/doc/diagrams/...`
